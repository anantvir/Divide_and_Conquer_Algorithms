...
def merge(A,p,q,r):
    ...
    R= list()
    for i in range(n_1):                    
        L.insert(i,A[i])                # Put elements from given array 'A' into newly created L and R arrays 
        print(L[i]) 
    for j in range(n_2):
        R.insert(j,A[q+j])
    L.append(None)                      # Sentinel 1
    R.append(None)                      # Sentinel 2
    i = 1
    j = 1
    for x in range(len(A)):             # because we know there are total r - p + 1 elements which is = len(A)
